...
% 这章怎么写？
% - 技术选型：语言，分层，项目结构
% - 每一个 module \ specialized framework 的实例化:
%     device, tracker, detector, recognizer
...
In this section, we describe how we create hot spot for those frozen spots
...
For the device module in the core framework, we describe how we instantiate it to
support three different kinds of depth cameras.
For detector specialized framework, we describe what kind of modification we
...
For recognizer specialized framework, we describe how we train a ReID network
which is a combination of the identification model and triplet model explain in
\autoref{sec:related-work-re-id-idm} and \autoref{sec:related-work-re-id-dism}.
For tracker specialized framework, we describe how we instantiate it with the
existing NiTE2 introduce in \autoref{sec:related_work_openiss_nite2}
implementation.
...
%\label{fw-recognizer-spec-dataset}
%
%Once we defined the network structure, the next step will be training the model
%on the network. But before we do that, let's take a look to the public
%available datasets in this domain. There are three datasets we are going to
%introduce which are employed in our implementation, shown as
...
%these three to allow our ReID algorithm can be trained and validated on.
%
%\begin{table}
%    \begin{center}
...
For the optimizer, we are using the build-in Adam algorithm provided by Keras
...
% %%%%%%% move to application chapter

%The workflow of this hot spot shown as \autoref{fig:fw-skeleton-workflow}.
...
%passing different indicator to the tracker factory.
%
%\begin{figure}
