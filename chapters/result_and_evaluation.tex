...
The skeleton tracking application that was described in \autoref{sec:fw-app-skt}
...
%Since this steps should not be worried by our system, we can probably
%get more powerful devices and display machines with higher speed 
%from the market easily.
...
In \autoref{sec:intro-non-func-req}, we have identified a non-functional 
requirement for
...
%Concretely, by means that the software we
%provided can at least achieve the goal we set up at the beginning, then further
%the system and its functionalities should also be easy to use from the user 
%point of view.
...
In term of the solution can be used with ease, currently, we don't have any
...
Since the P-R curve is often zigzag which is not easy for comparison, such as shown 
...
We perform comprehensive experiments on the person re-identification task since it
is the main goal of this thesis. Let us walk through them one by one. In order
to give the reader with a sense of how well our model perform, we provide the 
state-of-the-art result obtained from two common datasets shown as
...
We also trained the model in two different settings in the same Market1501
dataset. The result are expressed in \autoref{tab:eval-two-env}. Obviously,
...
identical. The difference is limited to 0.001 which is under expectation. 
...
\textbf{Comparison between the results obtained with the same dataset validation:}
As we mentioned in \autoref{sec:eval-recognizer-dataset}, we have an abstraction
layer of the ReID dataset which enables us to train on various datasets with
only a few modification (actually just passing different parameters).
By making use of it, we perform training and validation on three different
datasets with \texttt{setting 2}. We obtained the results shown by
\autoref{tab:eval-dif-datasets}. From the results, we found that the performance
on the CUHK03 dataset is poor. We thought that might be due to lesser training
...
the DukeMTMC-reID dataset, the result we have currently is under expectation.
